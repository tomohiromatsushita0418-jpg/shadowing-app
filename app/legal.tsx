...
//
// ⚠️ 公開前に TODO をすべて実在の情報で埋めてください。有料課金を始めるには
//    法律上この表記が必要で、Stripe の審査でもこのページの URL を求められます。
//
// 住所と電話番号は「請求があったら遅滞なく開示する」形での運用が実務上一般的です
// （そのままの文面を下に入れてあります）。自宅住所を出したくない場合はバーチャル
...
const TOKUSHOHO: Row[] = [
  ...
  { label: '無料で利用できる範囲', value: '最新の10エピソードは登録の有無にかかわらず無料でご利用いただけます。それ以降のエピソードおよび一部機能は有料プランのご購読が必要です。' },
...
const TERMS: { heading: string; body: string }[] = [
  ...
  },
  {
    ...
    body: '最新の10エピソードは無料でご利用いただけます。それ以降のエピソードおよび瞬間英作文・熟語帳等の一部機能は、有料プランのご購読によりご利用いただけます。無料の範囲では自動的に課金が始まることはありません。',
  },
  {
    heading: '第5条（禁止事項）',
